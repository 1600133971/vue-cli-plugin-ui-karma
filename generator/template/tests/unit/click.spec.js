...
describe("click.vue", () => {
  ...
  it("测试按钮点击事件", () => {
    ...
    setTimeout(done => {
      ...
      expect(vm.$el.querySelector(".init-num").textContent).to.equal(
        "初始值为11"
      );
      expect(vm.$el.querySelector(".click-num").textContent).to.equal(
        "点击了3次"
      );
      expect(vm.$el.querySelector(".result-num").textContent).to.equal(
        "最终结果为41"
      );
      done();
    }, 100);
